...
void Application::Start() {
    ...
    HexapodMcpTools::RegisterTools(mcp_server);

    // Initialize hexapod subsystems based on board type:
    // - hexapod_voicebot: UART bridge master (sends commands to Bot via UART)
    ...
    if (board_type == "hexapod_voicebot") {
        ...
        ESP_LOGI(TAG, "Hexapod Bot: UART bridge + HTTP server started (slave, port 8081)");
    } else if (board_type == "hexapod_dual") {
        HexapodUartBridge::GetInstance().Start(HexapodUartBridge::Role::kDual);
        HexapodServer::GetInstance().Start(8081);
        ESP_LOGI(TAG, "Hexapod Dual: UART bridge + HTTP server started (port 8081)");
    } else {
        ESP_LOGI(TAG, "No hexapod subsystems for board: %s", board_type.c_str());
    }
    ...
    protocol_->OnAudioChannelOpened([this, codec, &board]() {
        board.SetPowerSaveMode(false);
        if (codec != nullptr && protocol_->server_sample_rate() != codec->output_sample_rate()) {
...
bool Application::InitMusic() {
    ...
    ESP_LOGI(TAG, "InitMusic: online music player initialized with MCP tools");
    return true;
}
